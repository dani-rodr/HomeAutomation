...
public class LgDisplayTests : IDisposable
{
    ...
    public LgDisplayTests()
    {
        ...
        // Set initial state
        _mockHaContext.SetEntityState(_entities.LgWebosSmartTv.EntityId, "on");
    ...
    public void ShowSource_Should_MapDisplaySourceCorrectly(string sourceKey, string expectedHdmiSource)
    {
        // Arrange
        _mockHaContext.SetEntityState(_entities.LgWebosSmartTv.EntityId, "on");
    ...
    {
        // Arrange
        _mockHaContext.SetEntityState(_entities.LgWebosSmartTv.EntityId, "off");
    ...
    {
        // Arrange
        _mockHaContext.SetEntityAttributes(_entities.LgWebosSmartTv.EntityId, new { source = "HDMI 1" });
    ...
    {
        // Arrange
        _mockHaContext.SetEntityAttributes(_entities.LgWebosSmartTv.EntityId, new { source = "HDMI 3" });
    ...
    public void IsShowingLaptop_WhenCurrentSourceIsNotHdmi3_Should_ReturnFalse()
    {
        ...
        // Assert
        result.Should().BeFalse("IsShowingLaptop should return false when current source is not HDMI 3");
    }
    ...
    [Fact]
    public async Task SetBrightnessAsync_Should_SendCorrectWebOSTvCommand()
    {
        // Act
        ...
        commandCall.Should().NotBeNull("Should have called webostv command service");
        ...
        commandProperty
            ...
            ?.ToString()
            .Should()
            .Be("system.notifications/createAlert", "Should call createAlert command");
    }

    [Fact]
    public async Task SetBrightnessAsync_Should_SendEnterButtonAfterDelay()
    {
        // Act
        ...
        buttonCall.Should().NotBeNull("Should have called webostv button service");

        // Verify button parameter
        var buttonProperty = buttonCall!.Data?.GetType().GetProperty("button");
        buttonProperty?.GetValue(buttonCall.Data)?.ToString().Should().Be("ENTER", "Should press ENTER button");
    }

    [Fact]
    public async Task SetBrightnessAsync_WhenSameBrightness_Should_NotSendCommands()
    {
        // Arrange - Set initial brightness
        await _lgDisplay.SetBrightnessAsync(80);
        _mockHaContext.ClearServiceCalls();

        // Act - Try to set same brightness
        await _lgDisplay.SetBrightnessAsync(80);

        // Assert
        _mockHaContext.ServiceCalls.Should().BeEmpty("Should not send commands when brightness is already set");
    }

    [Fact]
    public async Task SetBrightnessAsync_Should_UpdateInternalBrightnessValue()
    {
        // Act
        await _lgDisplay.SetBrightnessAsync(65);
        _mockHaContext.ClearServiceCalls();

        // Second call with same value should not trigger commands
        await _lgDisplay.SetBrightnessAsync(65);

        // Assert
        _mockHaContext
            .ServiceCalls.Should()
            .BeEmpty("Internal brightness should be updated to prevent duplicate calls");
    }
    ...
    [InlineData(75)]
    [InlineData(100)]
    public async Task SetBrightnessAsync_Should_HandleValidBrightnessValues(int brightness)
    {
        // Act
    ...
    public async Task SetBrightnessAsync_Should_CreateCorrectBrightnessPayload()
    {
        ...
        // Verify payload structure exists (we can't easily test the exact structure without reflection
        // but we can verify the command was called with a payload)
        var payloadProperty = commandCall!.Data?.GetType().GetProperty("payload");
        payloadProperty?.GetValue(commandCall.Data).Should().NotBeNull("Command should include brightness payload");
    }
    ...
    public void ShowToast_Should_IncludeMessageInPayload()
    {
        ...
        var payloadProperty = commandCall!.Data?.GetType().GetProperty("payload");
        payloadProperty?.GetValue(commandCall.Data).Should().NotBeNull("Command should include message payload");
    }
    ...
    public void EntityId_Should_ReturnCorrectMediaPlayerEntityId()
    {
        ...
        // Assert
        entityId.Should().Be("media_player.lg_webos_smart_tv", "Should return LG WebOS Smart TV entity ID");
    }
    ...
    {
        // Arrange
        var stateChanges = new List<StateChange<MediaPlayerEntity, EntityState<MediaPlayerAttributes>>>();

        // Act
        _lgDisplay.StateChanges().Subscribe(stateChanges.Add);

        // Simulate state change
        _mockHaContext.SimulateStateChange(_entities.LgWebosSmartTv.EntityId, "off", "on");
    ...
    public void SetVolume_Should_DelegateToMediaPlayer()
    {
        ...
        // Assert
        _mockHaContext.ShouldHaveCalledService("media_player", "volume_set", _entities.LgWebosSmartTv.EntityId);
    }
    ...
    private class TestWebostvServices(IHaContext haContext)
    {
        ...
        {
            _haContext.CallService("webostv", "button", null, new { entity_id = entityId, button = button });
        }
    }
