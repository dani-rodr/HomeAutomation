...
public static class StateChangeObservableExtensions
{
    ...
    public static IObservable<StateChange> WhenStateIsForSeconds(
        ...
        string desiredState,
        int time) => source.WhenStateIsFor(s => s?.State == desiredState, TimeSpan.FromSeconds(time), Scheduler.Default);

    public static IObservable<StateChange> WhenStateIsForMinutes(
        this IObservable<StateChange> source,
        string desiredState,
        int time) => source.WhenStateIsFor(s => s?.State == desiredState, TimeSpan.FromMinutes(time), Scheduler.Default);

    public static IObservable<StateChange> WhenStateIsForHours(
        this IObservable<StateChange> source,
        string desiredState,
        int time) => source.WhenStateIsFor(s => s?.State == desiredState, TimeSpan.FromHours(time), Scheduler.Default);
