...
The installer creates the LXC, installs the required toolchain, then tells you how to enter the container and run the interactive application setup.
...
Run the wizard from inside the container:
...
The base container setup enables autologin on the Proxmox LXC console getty services, so the Proxmox Console button should open straight into a root shell.
